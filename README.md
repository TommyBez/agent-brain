...
Agents call `search` and `context` with plain text. The server generates the query embedding through AI Gateway using `openai/text-embedding-3-small` with 1536 dimensions, matching the page embeddings generated by the separate runner. Advanced clients can still supply their own matching query vectors, which bypass the provider call. The server validates dimensions, model and page version.
