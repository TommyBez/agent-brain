...
3. **Git export:** independently of the embedding result, the workflow exports a consistent database snapshot as Markdown pages, `graph.json` and a manifest to the private export repository. GitHub's API creates a daily commit and updates the branch without force. Per-job receipts and remote readback make retries safe. A job succeeds only after its commit is reachable from the remote branch. Export makes no AI calls.

Vercel Workflow retries transient step failures. Completed and partial jobs are not repeated for the same UTC date. **Operations → Run maintenance** starts today's work or retries failed jobs; the page shows results, consolidation reports, export commits and errors. The owner-session-only `POST /api/operations` endpoint supports that action with an empty JSON object. GitHub holds the exported files and history; it runs no scheduled maintenance.
